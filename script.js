const inventors = [
  ...
  const countInstances = data.reduce((obj, item) => {
    if (!obj[item]) {
      obj[item] = 1;
    } else {
      obj[item]++;
  ...
  const hasAdult = devs.some(dev => (new Date().getFullYear() - dev.year) >= 19);
console.log(hasAdult);
  ...
  const allAdults = devs.every(dev => (new Date().getFullYear() - dev.year) >= 19);
